...
public class Heap<T extends Comparable<T>> {
    ...
    public T peek() {
    	...
    	T peak = container.get(0);
    	for (int i = 0; i < container.size(); ++i) {
    		if (container.get(i).compareTo(peak) < 0)
    			peak = container.get(i);
    	}
    ...
    public T poll() {
    	...
    	T last = container.get(0);
    	for (int i = 0; i < container.size(); ++i) {
    		if (container.get(i).compareTo(last) > 0)
    			last = container.get(i);
    	}
    	
    	container.set(container.indexOf(first) , last); 	
    ...
    private void heapifyUp() {
        ...
        while (hasRight(pos)) {
            int greaterParentIndex = getRightIndex(pos);
            if (hasLeft(pos) && container.get(getLeftIndex(pos)).compareTo(container.get(getRightIndex(pos))) > 0) {
                greaterParentIndex = getLeftIndex(pos);
            }
            if (container.get(pos).compareTo(container.get(greaterParentIndex)) > 0) {
                break;
            } else {
                swap(pos, greaterParentIndex);
            }
            pos = greaterParentIndex;
        }
    }
